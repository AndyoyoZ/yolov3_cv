...
"{ source       |                    | image or video for detection        }" 
...
int main(int argc, char** argv)
{
    ...
    std::string classesFile = "../yolo_cfg/coco.names"; 
    ...
    cv::String modelConfiguration = "../yolo_cfg/yolov3.cfg";
    cv::String modelWeights = "../yolo_cfg/yolov3.weights";
...
while (1)
{
    ...
    cv::putText(frame, label, cv::Point(0, 15), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0));
...
void drawPred(int classId, float conf, int left, int top, int right, int bottom, cv::Mat& frame)
{
    ...
    cv::putText(frame, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0,0,255));
}
